...
Configuration lives in `backend/.env` (gitignored). It is read by a hand-rolled loader in `config.py` (no `python-dotenv`). Env vars always override `.env`. Key vars: `LLM_PROVIDER`, `LLM_MODEL`, `LLM_API_KEY`, `LLM_API_BASE`, `LLM_TEMPERATURE`, `LLM_MAX_TOKENS`, `AGENT_MAX_ITERATIONS`, `AGENT_MAX_HISTORY`, `AGENT_SOURCE_MODE` (`direct`/`subagent` — single- vs multi-agent retrieval), `AGENT_CONTEXT_COMPACTION` (`compact`/`elide`), and `LANGFUSE_*`. **If `LLM_API_KEY` is unset, the app degrades gracefully** — `agent`/`llm` are `None` and `/api/chat*` fall back to a rule-based reply (`_generate_reply` in `main.py`).
...
- **`context.py` + `compaction.py` — context management.** Two modes via `AGENT_CONTEXT_COMPACTION`: `compact` (default) folds old history into a **deterministic template summary** (seven-section, no LLM call) once the token budget (`AGENT_CONTEXT_TOKEN_BUDGET`, default 24000) is exceeded, then **freezes** the split point until the next trigger — keeping the prompt prefix byte-stable between triggers so DeepSeek prefix caching keeps hitting. Split points never orphan a `tool` message from its `tool_calls` assistant (boundary-safe walk-back); re-compaction merges summaries with priority-based trimming. Compaction state persists on the Session (`compaction` field); `session.messages` stays append-only. `elide` mode is the legacy fallback (placeholder-elides stale tool results per round; cache-hostile, kept for comparison).
- **`session.py`** — one JSON file per session under `backend/data/sessions/`. Stores full message history (incl. tool calls/results) for session restore, plus the optional `compaction` state (summary + frozen split point).
